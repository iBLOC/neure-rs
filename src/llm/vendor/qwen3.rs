...
/// Convert from the upstream qwen3::Config (no use_flash_attn field)
/// to the vendored one with flash-attn enabled.
impl From<candle_transformers::models::qwen3::Config> for Config {
    fn from(cfg: candle_transformers::models::qwen3::Config) -> Self {
        Self {
...
impl Qwen3Attention {
    ...
    pub(crate) fn forward(
        ...
        let ctx = if self.use_flash_attn {
            candle_flash_attn::flash_attn(&q, &k, &v, None, true)?
        } else {
